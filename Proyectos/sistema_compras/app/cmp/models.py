...
@receiver(post_delete, sender=ComprasDet)  #cuando borra y el modelo
def detalle_compra_borrar(sender,instance, **kwargs):
    id_producto = instance.producto.id #recupera el id que se elimono
    id_compra = instance.compra.id #lo mismo

    enc = ComprasEnc.objects.filter(pk=id_compra).first()
    if enc:
        sub_total = ComprasDet.objects.filter(compra=id_compra).aggregate(Sum('sub_total')) #actualiza los totales
